// Third-party module
const { ActionRowBuilder, ChatInputCommandInteraction, EmbedBuilder, SelectMenuBuilder, SlashCommandBuilder } = require('discord.js'); // Elements from the discord.js library
...
module.exports = {
    data: new SlashCommandBuilder()
        .setName('moyennes')
        .setDescription('Calculer tes moyennes par unité ou par matière.'),
    ...
    async execute(interaction) {
        ...
        if (generalAverage === null) { // This appends when the student has not added any mark or only absences
        ...
        const missingMarks = student.marks.length < publishedMarkCount
            ? `⚠️ *Tu n'as pas ajouté toutes les notes publiées (${student.marks.length}/${publishedMarkCount}),\nUtilise la commande* ${commandMention(interaction.client, 'notes manquantes')} *pour voir lesquelles.*\n\n`
            : '';
