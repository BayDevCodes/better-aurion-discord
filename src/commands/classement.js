  ...
  async execute(interaction) {
    ...
    if (subCommand === 'moyennes') {
      ...
      let description = studentRank // This is equal to 0 if the student is not part of the rankings
        ...
        : (await Promotion.get(interaction.user.id)).marks.length === publishedMarkCount // Check if the student has added all published marks
    ...
    if (!publishedMark) {
      const unknownEmbed = new EmbedBuilder()
        ...
        .setColor('Red')
        .setDescription(
          `L'identifiant \`${markId}\` ne correspond à aucune note publiée.\n***Si** c'est pourtant le cas, utilise* ${commandMention(
            interaction.client,
            'aled'
          )}`
        );
    ...
    if (rankings.length === 0) {
      const noStudentsEmbed = new EmbedBuilder()
        ...
        .setTitle('Classement indisponible')
        .setDescription(
          `Aucun·e étudiant·e n'a ajouté cette note, utilise ${commandMention(
            interaction.client,
            'notes saisir'
          )} pour ajouter la tienne`
        );
    ...
    const rankingEmbed = new EmbedBuilder()
      ...
      .setDescription(description)
      .setFooter({
        text: `Moyenne de la promo: ${Math.round((sum / rankings.length) * 100) / 100} (sur ${
          rankings.length
        } étudiant·e·s classé·e·s)`,
      }); // Calculate the promotion's average for this mark
