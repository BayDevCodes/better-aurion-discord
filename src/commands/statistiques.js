...
const studentCount = require('../constants/mails.json').students.length; // Array of student mails' first parts

// Export the command's data & execute function
module.exports = {
  data: new SlashCommandBuilder().setName('statistiques').setDescription('Qui regarde ça en vrai?'),
  ...
  async execute(interaction) {
    ...
    description += `> Interactions totales: \`${
      (await Main.get('interactionCount')) || Number(0)
    }\`\n`;
    description += `> Étudiants enregistrés: \`${
      (await Promotion.all()).length
    }/${studentCount}\`\n`;
