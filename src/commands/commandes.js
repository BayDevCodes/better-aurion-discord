...
module.exports = {
    ...
    async execute(interaction) {
        ...
        for (const command of interaction.client.commands) { // Do this for each command loaded in the current instance
            /** @type {SlashCommandBuilder} */
            const builder = command[1].data;
            if (builder.name === 'dev' && interaction.user.id !== interaction.client.application.owner.id) continue; // Show the "dev" command to the appplication manager only
