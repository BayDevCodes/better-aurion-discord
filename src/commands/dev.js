...
module.exports = {
      ...
      subcommand
        ...
        .addUserOption((option) =>
          option
            .setName('utilisateur')
            .setDescription(
              "L'étudiant·e à promouvoir (s'il·elle n'apparaît pas dans la liste, utilise son ID)."
            )
            .setRequired(true)
        )
    )
      ...
      subcommand
        ...
        .addUserOption((option) =>
          option
            .setName('utilisateur')
            .setDescription(
              "L'étudiant·e à reléguer (s'il·elle n'apparaît pas dans la liste, utilise son ID)."
            )
            .setRequired(true)
        )
    ),
  ...
  async execute(interaction) {
    ...
    if (interaction.user.id !== ownerId) {
      // This command is restricted to the application manager
    ...
    if (interaction.options.getSubcommand() === 'promouvoir') {
      if (referee) {
        const alreadyRefereeEmbed = new EmbedBuilder()
          ...
          .setColor('Orange')
          .setDescription(
            `*L'utilisateur* **${user.username}** *est déjà référent·e de cette instance*`
          );
      ...
      const newRefereeEmbed = new EmbedBuilder()
        ...
        .setDescription(
          `*L'utilisateur* **${user.username}** *est désormais référent·e de cette instance*`
        );
    ...
    if (!referee) {
      const notRefereeEmbed = new EmbedBuilder()
        ...
        .setColor('Orange')
        .setDescription(
          `*L'utilisateur* **${user.username}** *n'est pas référent·e de cette instance*`
        );
    ...
    const removedRefereeEmbed = new EmbedBuilder()
      ...
      .setDescription(
        `*L'utilisateur* **${user.username}** *n'est plus référent·e de cette instance*`
      );
