...
module.exports = {
    ...
    async execute(interaction) {
        ...
        for (const unitId of units) {
            /** @type {String} */
            const unitName = marks.names.units[unitId];
            const goal = Math.round(interaction.options.getNumber(unitName.split(' ')[0].toLowerCase()) * 100) / 100; // Round the input value to 2 decimal digits if any

            description +=`> ${unitName}: \`${student.goals[unitId]}\`${goal ? ` -> \`${goal}\`` : ''}\n`; // Format each unit line
