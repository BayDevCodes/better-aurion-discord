...
module.exports = {
  data: new StringSelectMenuBuilder().setCustomId('chooseModule'),
    ...
    switch (embed.data.title.split(' ')[0]) {
      case 'Classement':
        ...
        const rankings = (await Promotion.all())
          .filter(
            (s) =>
              ...
              s.value.averages[unitId][moduleId].self !== null
          )
          .sort(
            (a, b) =>
              b.value.averages[unitId][moduleId].self - a.value.averages[unitId][moduleId].self
          );
        if (!rankings.length) {
          embed
            .setColor('Orange')
            .setDescription("Personne n'a encore de moyenne pour ce module...")
            .setFooter({ text: ' ' });
        ...
        let description = studentRank // This is equal to 0 if the student is not part of the rankings
          ? `Tu es à la ${studentRank}è${studentRank === 1 ? 're' : 'me'} place avec \`${
              rankings[studentRank - 1].value.averages[unitId][moduleId].self
            }\`\n`
          ...
          ? `*Tu n\'es pas classé·e car tu es noté·e pour aucune évaluation de ce module.*\n`
        ...
        for (i = 0; i < 10; i++) {
          ...
          }** avec \`${student.value.averages[unitId][moduleId].self}\`\n`;
        }
      ...
      case 'Moyenne':
        ...
        if (moduleAverages.self === null) {
          embed
            .setColor('Orange')
            .setDescription("Tu n'as pas encore de moyenne pour ce module...")
            .setImage();
          return interaction.update({
            components: [interaction.message.components[1]],
            embeds: [embed],
        ...
        const promotionValues = promotionAverages // Are promotion averages available?
          ? Object.keys(promotionAverages[unitId][moduleId])
              .slice(0, -1)
              .map((typeId) => promotionAverages[unitId][moduleId][typeId])
          : [];
        ...
        file = await averagesChart(names, values, promotionValues, 'type'); // Generate the Quickchart attachment
        embed
          .setDescription(
            `${missingMarks}*Ta moyenne en ${marks.names.modules[moduleId]} est de \`${
              moduleAverages.self
            }\`${
              promotionAverages && promotionAverages[unitId][moduleId].self !== null
                ? ` [promo: \`${promotionAverages[unitId][moduleId].self}\`]`
                : ''
