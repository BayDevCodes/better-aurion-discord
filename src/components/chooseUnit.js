...
module.exports = {
  data: new StringSelectMenuBuilder()
    ...
    .addOptions(
      Object.keys(marks.weights).map((unitId) =>
        new StringSelectMenuOptionBuilder().setLabel(marks.names.units[unitId]).setValue(unitId)
      )
    ),
  ...
  async execute(interaction) {
    ...
    switch (embed.data.title.split(' ')[0]) {
      case 'Classement':
        ...
        const rankings = (await Promotion.all())
          .filter(
            (s) =>
              s.value.marks.length === publishedMarkCountR && s.value.averages[unitId].self !== null
          )
      ...
      case 'Coefficients':
        ...
        embed
          .setTitle(`Coefficients en ${marks.names.units[unitId]}`)
          .setImage(weightsChart(unitId)); // Update chart with unit's module weights
        break;
      ...
      case 'Moyenne':
        ...
        if (unitAverages.self === null) {
          embed
            .setColor('Orange')
            .setDescription("Tu n'as pas encore de moyenne pour cette unité...")
            .setImage();
          return interaction.update({
            components: [new ActionRowBuilder().addComponents(backButton)],
            embeds: [embed],
    ...
    const rows = [
      new ActionRowBuilder().addComponents(moduleSelectMenu),
