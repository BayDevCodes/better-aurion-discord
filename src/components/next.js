...
module.exports = {
    ...
    async execute(interaction) {
        ...
        if (currentPage === 1) row.components.unshift(interaction.client.components.get('previous').data); // Add the "previous" button if coming from the first page
        ...
        const pageContent = await Promise.all(studentMarks.slice(currentPage * 10, currentPage * 10 + 10).map(async mark => `> \`${mark.id}\` *${await Marks.get(mark.id)}* **${mark.value < 0 ? 'non noté·e' : `avec ${mark.value}`}**`));
