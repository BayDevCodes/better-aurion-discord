...
module.exports = {
  ...
  async execute(interaction) {
    ...
    let file; // Initialize a potential chart file

    // If we are in the module scope
    if (moduleName) {
      /** @type {StringSelectMenuBuilder} */
      const moduleSelectMenu = interaction.client.components.get('chooseModule').data;
      const unitId = Object.keys(marks.weights).find(
        (unitId) => marks.names.units[unitId] === unitName
      ); // Get the unit
      ...
      switch (titleStart.split(' ')[0]) {
        case 'Classement':
          ...
          const rankings = (await Promotion.all())
            .filter(
              (s) =>
                s.value.marks.length === publishedMarkCountR &&
                s.value.averages[unitId].self !== null
            )
            .sort((a, b) => b.value.averages[unitId].self - a.value.averages[unitId].self);
          if (!rankings.length) {
            embed
              .setColor('Orange')
              .setDescription("Personne n'a encore de moyenne pour cette unité...")
              .setFooter({ text: ' ' });
          ...
          let description = studentRank // This is equal to 0 if the student is not part of the rankings
            ? `Tu es à la ${studentRank}è${studentRank === 1 ? 're' : 'me'} place avec \`${
                rankings[studentRank - 1].value.averages[unitId].self
              }\`\n`
          ...
          for (i = 0; i < 10; i++) {
            ...
            description += `**${i + 1}** - **${
              student.value.anonymous
                ? '🕵️ Anonyme'
                : `[${nameFromEmail(student.value.email)}](https://discordapp.com/users/${
                    student.id
                  })`
            }** avec \`${student.value.averages[unitId].self}\`\n`;
          }
          ...
          embed
            ...
            .setFooter({
              text: `${rankings.length} étudiant·e·s classé·e·s (ayant saisi toutes les notes)`,
            });
          break;
        ...
        case 'Moyenne':
          ...
          const promotionValues = promotionAverages // Are promotion averages available?
            ? Object.keys(promotionAverages[unitId])
                .slice(0, -1)
                .map((moduleId) => promotionAverages[unitId][moduleId].self)
            : [];

          const publishedMarkCountA = (await Marks.all()).length;
          const missingMarks =
            student.marks.length < publishedMarkCountA
          ...
          file = await averagesChart(names, values, promotionValues, 'module'); // Generate the Quickchart attachment
          embed
            .setDescription(
              `${missingMarks}*Ta moyenne en ${marks.names.units[unitId]} est de \`${
                unitAverages.self
              }\`${
                promotionAverages && promotionAverages[unitId].self !== null
                  ? ` [promo: \`${promotionAverages[unitId].self}\`]`
                  : ''
      ...
      moduleSelectMenu.setOptions(
        Object.keys(marks.weights[unitId])
          .slice(1)
          .map((moduleId) =>
            new StringSelectMenuOptionBuilder()
              .setLabel(marks.names.modules[moduleId])
              .setValue(moduleId)
          )
      );

      return interaction.update({
        components: [new ActionRowBuilder().addComponents(moduleSelectMenu)].concat(
          interaction.message.components
        ),
        embeds: [embed],
        files: file ? [file] : [], // Include the chart attachment so it can be displayed
      }); // Add the module select menu
    }
    ...
    switch (titleStart.split(' ')[0]) {
      case 'Classement':
        ...
        const rankings = (await Promotion.all())
          .filter((s) => s.value.marks.length === publishedMarkCountR)
        ...
        if (rankings.length === 0) {
          embed
            ...
            .setFooter({ text: ' ' })
            .setDescription(
              `Aucun·e étudiant·e n'a ajouté toutes ses notes, utilise ${commandMention(
                interaction.client,
                'notes manquantes'
              )} pour voir celles que tu dois ajouter`
            );
        ...
        let description = studentRank // This is equal to 0 if the student is not part of the rankings
          ? `Tu es à la ${studentRank}è${studentRank === 1 ? 're' : 'me'} place avec \`${
              rankings[studentRank - 1].value.averages.general
            }\`\n`
          : (await Promotion.get(interaction.user.id)).marks.length === publishedMarkCountR // Check if the student has added all published marks
          ? `*Tu n\'es pas classé·e car tu es noté·e pour aucune évaluation.*\n`
          : `⚠️ *Tu n'as pas ajouté toutes les notes disponibles,\nutilise* ${commandMention(
              interaction.client,
              'notes manquantes'
            )} *pour voir lesquelles.*\n`;
        ...
        for (i = 0; i < 10; i++) {
          ...
          }** avec \`${student.value.averages.general}\`\n`;
        }
      ...
      case 'Moyenne':
        ...
        embed
          ...
          .setDescription(
            `${missingMarks}*Ta moyenne générale est de \`${student.averages.general}\`${
