...
module.exports = {
  ...
  async execute(interaction) {
    ...
    if (currentPage === maxPage)
      row.components.push(interaction.client.components.get('next').data); // Add the "next" button if coming from the last page
    ...
    const pageContent = await Promise.all(
      studentMarks
        .slice(currentPage * 10 - 20, currentPage * 10 - 10)
        .map(
          async (mark) =>
